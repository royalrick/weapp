...
type SubscribeMessage struct {
	...
	// 组成完整的 URL 地址
	// 默认包含 AccessToken
	conbineURI func(url string, queries map[string]string) (string, error)
}

func NewSubscribeMessage(request *request.Request, conbineURI func(url string, queries map[string]string) (string, error)) *SubscribeMessage {
	sm := SubscribeMessage{
		request:    request,
...
func (cli *SubscribeMessage) AddTemplate(tid, desc string, keywordIDList []int32) (*AddTemplateResponse, error) {
...
//
// pid 模板ID
func (cli *SubscribeMessage) DeleteTemplate(pid string) (*request.CommonError, error) {
	api, err := cli.conbineURI(apiDeleteTemplate, nil)
	if err != nil {
		return nil, err
	}
...
func (cli *SubscribeMessage) GetTemplateCategory() (*GetTemplateCategoryResponse, error) {
	api, err := cli.conbineURI(apiGetTemplateCategory, nil)
	if err != nil {
		return nil, err
	}
...
// tid 模板ID
func (cli *SubscribeMessage) GetPubTemplateKeyWordsById(tid string) (*GetPubTemplateKeyWordsByIdResponse, error) {
	queries := map[string]string{
		"tid": tid,
	}
	uri, err := cli.conbineURI(apiGetPubTemplateKeyWordsById, queries)
	if err != nil {
		return nil, err
	}
...
func (cli *SubscribeMessage) GetPubTemplateTitleList(ids string, start, limit int) (*GetPubTemplateTitleListResponse, error) {
	...
	uri, err := cli.conbineURI(apiGetPubTemplateTitleList, queries)
	if err != nil {
		return nil, err
	}
...
// Send 发送订阅消息
func (cli *SubscribeMessage) SendSubscribeMsg(msg *SubscribedMessage) (*request.CommonError, error) {
	api, err := cli.conbineURI(apiSendSubscribeMessage, nil)
	if err != nil {
		return nil, err
	}
