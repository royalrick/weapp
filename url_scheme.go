...
type URLSchemeRequest struct {
	...
	ExpireType     uint8 `json:"expire_type,omitempty"`     //默认值0，到期失效的 scheme 码失效类型，失效时间：0，失效间隔天数：1
	ExpireInterval uint8 `json:"expire_interval,omitempty"` //到期失效的 scheme 码的失效间隔天数。生成的到期失效 scheme 码在该间隔时间到达前有效。最长间隔天数为30天。is_expire 为 true 且 expire_type 为 1 时必填
}
